    ...
    def fetch_adjust_factors(self, code: str, start_date: str = "1990-01-01",
        ...
        while rs.next():
            ...
            rows.append({
                ...
                "fore_factor": float(row[2]) if row[2] else None,       # foreAdjustFactor (累计)
                "adj_factor_after": float(row[3]) if row[3] else None,  # backAdjustFactor (累计)
                "adj_factor": float(row[4]) if row[4] else None,        # adjustFactor
            })

        if rows:
            self.db.executemany(
                "INSERT OR REPLACE INTO adjust_factor (code, trade_date, adj_factor, adj_factor_after, fore_factor) "
                "VALUES (:code, :trade_date, :adj_factor, :adj_factor_after, :fore_factor)",
                rows,
            )

    def fetch_all_adjust_factors(self, start_date: str = "1990-01-01", force: bool = False):
        """拉取全部股票复权因子（已有 fore_factor 的跳过，force=True 全部重拉）"""
        codes = self.db.get_active_stock_codes()
        total = len(codes)
        ...
        for i, code in enumerate(codes):
            if not force:
                ...
                existing = self.db.fetchone(
                    "SELECT COUNT(*) AS c FROM adjust_factor WHERE code = ? AND fore_factor IS NOT NULL",
                    (code,)
                )
